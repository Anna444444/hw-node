...
export class Controller {
    ...
    generateConfirmCode = (req, res) => {
        const confirmCode = Math.floor(Math.random() * 10000);
        console.log('Confirmation Code:', confirmCode);
        res.status(200).send('Confirmation code generated');
    }

    checkConfirmCode = (req, res) => {
        const confirmCode = req.body.code;
        if (confirmCode === expectedConfirmCode) {
            res.status(200).send('Confirmation code is valid');
        } else {
            res.status(400).send('Confirmation code is invalid');
        }
    }
}
