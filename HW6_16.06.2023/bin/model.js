...
export class Database {
    ...
    async addUser(name, telegramId, chatId, telegramUrl = null) {
    ...
    async addEvent(name, city, address, date, time, isRegular, price, contact, orgId, posterUrl = null) {
    ...
    async test() {
        let query = `INSERT INTO Users (name, telegram_id, chat_id) VALUES("AMIR", 23452, 23452345)`;
        ...
        query = `INSERT INTO Events (name, city, date, org_id) VALUES('Концерт AC/DC', "Almaty", '2023-06-17', 1)`;

        try {
            await this.db.exec(query);
        } catch {
            console.log('Такой Event уже есть!');
        }
